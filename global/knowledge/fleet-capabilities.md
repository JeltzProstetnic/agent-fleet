...
The fleet runs whichever Opus version `mclaude` (via `cc-mirror`) bundles at install time. The relevant models today (Fable 5 — id `claude-fable-5`, a tier above Opus — is selectable from this fleet; whether it is billed or subscription-included depends on your plan, so gate its use per-project yourself — see the Fable 5 section below):
...
| **Fable 5** | Selectable, not a default (id `claude-fable-5`) | — | — | Preview flagship (2026-06-09), a tier above Opus. API list price $10/M in, $50/M out; subscription inclusion has changed repeatedly, so treat cost as current-state and re-confirm rather than assuming. **Recommended:** gate it per-project — free rein on the projects where deep reasoning pays for itself, ask first everywhere else. See the Fable 5 section below. |
...
> **Access is account-dependent and has changed more than once.** The `fable` subagent (`model: claude-fable-5`) and `--model fable` are selectable from this fleet, but whether a call is metered or subscription-included depends on your plan and on Anthropic's current deployment — see the pricing timeline below. **Suggested per-project gating:** nominate the few projects where the hardest reasoning genuinely pays (architecture, red-teaming, risky refactors) and let them route to Fable freely; require an explicit ask everywhere else. Record that decision in each project's `CLAUDE.md`. Treat both "it's free" and "it's unavailable" as current-state claims to re-verify by launching the agent, never as standing facts.
...
- **Pricing/credits:** launched 06-09 → free on Pro/Max/Team/Enterprise **June 9–22** → metered from **June 23** (first window). **Re-deployed globally 2026-07-01: subscription-included up to 50% of weekly usage limits through 2026-07-07; metered usage credits (API rates) from 2026-07-08.** Anthropic aims to restore it to subscription plans later — attributes this to **capacity**, not public pressure. API access unaffected throughout. Source: `anthropic.com/news/redeploying-fable-5`. (Separate, do-not-conflate: a **June 15** change moves Agent SDK / `claude -p` headless / GH-Actions usage to a separate monthly credit pool.) **Because this has flipped several times, verify the current billing state against your own plan before relying on it** — and prefer per-project gating over a fleet-wide hold, so the projects that need the reasoning keep it.
...
### What to route to Fable vs Opus (content-classifier caveat)

Empirical, from a 2026-07-05 session (parallel-subagent + Fable review of hand-rolled D-Bus/SNI code):
- **Fable is high-leverage on protocol/systems code** — D-Bus, SNI, IPC, and other hand-rolled protocol implementations benefit enormously from an independent Fable review before deploy (it caught a ship-blocking dual-session-bus bug + two MED bugs in ~323 LOC, then self-corrected on a second pass).
- **Route Fable AWAY from consciousness-theory and AI-architecture content.** Those trip Fable's content classifier (high-risk-domain auto-fallback / degraded engagement). For that material use **general-purpose + `model: opus`**, not Fable. Do NOT generalize the "Fable review works great" result across into those domains.
- **Parallel non-colliding subagents = high leverage on truly independent files.** Constraints that made it work: strict shared prompt (use Edit tool, no sed/awk/Bash writes; no commit/push/PR; no deploy/ssh; in-memory DB only), and non-overlapping file sets (never two subagents on one file).

(Fable's per-project ASK gate still applies — see the Fable 5 section above.)
...
| Personas | A default persona plus optional alternates with their own activation triggers; Day/Night mode (auto-switches at a configurable evening hour) | `foundation/personas.md` |
...
Capability is not the only constraint — physical ergonomics matter. Default coding and typing-intensive work to machines with a real keyboard and a desk. Handheld or docked-console machines are poor hosts for keyboard-intensive sessions unless a keyboard is actually attached; they suit read-only work, quick-touch checks, observation and ambient sessions, or work driven remotely from another machine over SSH. Record each machine's ergonomic role in its machine file so sessions can route work accordingly.
