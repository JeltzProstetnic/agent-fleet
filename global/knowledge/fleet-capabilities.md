...
The fleet runs whichever Opus version `mclaude` (via `cc-mirror`) bundles at install time. The relevant models today (Fable 5 — id `claude-fable-5`, a tier above Opus — is **selectable from this fleet and ✅ free within the user's Max sub as of 2026-07-16**; gated per-project — `crucible`/`aIware` free, all others ask the user first — see the Fable 5 section below):
...
| **Fable 5** | ✅ **Available & free within Max sub (2026-07-16)** — verified reachable WSL (id `claude-fable-5`) | — | — | **✅ FREE within the user's Max subscription as of 2026-07-16 (user directive) — supersedes the 2026-07-08 cost-hold (lifted).** Preview flagship (2026-06-09), a tier above Opus. **⚠ Per-project gating:** `crucible` and `aIware` may use freely; **every other project must ASK the user first.** NOT geo-blocked; API list price $10/M in, $50/M out. "Currently" free — re-confirm if the subscription window changes. See the Fable 5 section below. |
...
> **✅ AVAILABLE & FREE WITHIN THE USER'S MAX SUB (user directive 2026-07-16):** The `fable` subagent (`model: claude-fable-5`) and `--model fable` work from this fleet at no metered cost within the Max subscription. This **lifts and supersedes** the 2026-07-08 cost-hold (and the older 2026-06-17 "geo-blocked/unavailable" note — both stale). It is NOT geo-blocked. **⚠ Per-project gating (updated 2026-07-22, per MG 2026-07-21 "use it at your own discretion"):** `crucible`, `aIware`, and `cfg-agent-fleet` may route to Fable freely for critical/complex/risky work; **every other project must ASK the user before using Fable.** Availability is current-state ("currently" free) — if anything signals the subscription window changed, re-confirm with the user rather than assuming. Treat any future "unavailable" claim as a hypothesis to test by launching the agent, never a standing fact.
...
Empirical, from p0rn Deck-2 2026-07-05 (parallel-subagent + Fable review of hand-rolled D-Bus/SNI code):
