...
Not every trigger needs a full audit. Choose depth based on risk and trigger context.

| Level | Name | When | Scope | Duration |
|-------|------|------|-------|----------|
| **L1** | Quick Scan | Post-routine-session, daily check, low-risk trigger | Automated checks only: test suite, `sync.sh status`, git log consistency, invariant check | ~15 min |
| **L2** | Standard Audit | Post-multi-session work, post-deployment, moderate-risk trigger | L1 + manual review of session claims, cross-machine consistency, propagation verification | 1-2 hours |
| **L3** | Deep Audit | Post-unsupervised multi-day work, post-incident, high-risk trigger, user-requested | L2 + independent reproduction, full root cause analysis, sibling search, meta-audit, evidence file | Half-day+ |

**Default depth selection:**
- Resuming after 1 unsupervised session → L1
- Resuming after 3+ unsupervised sessions → L2
- User says "audit" / "check your work" → L2 unless scope suggests L3
- Post-incident, cross-machine, or after propagation changes → L3
...
a. **Identify time boundaries** — which date range, which sessions?
   - **Trust calibration per source:** Unsupervised sessions have lower trust than reviewed ones. Sessions on machines with different configs can produce different results from identical code. Rate each source's reliability before interpreting its claims.
...
d. **Identify actors** — which sessions on which machines?
...
g. **Check against consistency** — cross-reference claims vs reality ("all tests green" is a hypothesis, not a fact). Also check for contradictions between decisions.md, CLAUDE.md, and session claims.
...
j. **Check coverage gaps** — what WASN'T checked? Widen search around each finding.
...
a. **Sibling search** — for each finding, check what SIMILAR issues might exist.
...
d. **Confidence map** — rate confidence for each finding AND each "clean" area.
...
c. **Cross-reference against original claims** — every claim must be individually confirmed or refuted.
...
d. **Propagate** — template, hooks, all chains from dependency-map.md.
e. **Final `sync.sh check`** — must be clean.
f. **Update audit log** — record what was fixed and how.
g. **Verify prior corrective actions** — check if fixes from previous audits are still effective.
...
- If context runs low mid-audit, persist ALL conversation artifacts to files BEFORE the session ends.
- The next session must be able to continue the audit with ZERO information loss.
- This rule applies to ALL sessions during an audit, not just the first.
- Corollary: prefer over-persisting to under-persisting during audits.
...
6. **Trend analysis:** Compare quantitative metrics across audits.
7. **Control testing (fault injection):** Periodically verify that 1st/2nd line controls actually fire.
8. **Maturity self-rating:** Annually rate each audit capability on a 1-5 scale (CMMI-inspired).
...
- After any session that modified propagation targets (template, hooks)
