...
8. **Cross-session task handoff:** Add `## Next Session Task` to session-context.md with `task: true`, `file:`, `backlog:`, and `description:`. Rotation extracts it to `next-session-task.md`. Handoff tasks must reference a backlog ID in the `backlog:` field — if none exists, create the item (default P1) before writing the handoff. **HARD RULE: `file:` must NEVER point to `session-context.md`** — rotation blanks it, so the next session would find an empty file. Always write handoff data to a dedicated file in `docs/` (e.g., `docs/pending-<task>.md`) and point `file:` there. Never use `tmp/` — handoff files must persist across machines via git. This includes task lists, pending updates, execution plans — anything the next session needs to act on. **Restart-required verifications ARE handoff tasks** — if a fix can only be verified after restart (MCP changes, auth changes, hook changes), write it to next-session-task.md with explicit verification steps. Never put restart verifications in freeform "Recovery/Next session" text alone.
