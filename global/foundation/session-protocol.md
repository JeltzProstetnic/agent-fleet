...
8. **Cross-session task handoff:** Add `## Next Session Task` to session-context.md with `task: true|false`, `file:`, `backlog:`, and `description:`. Rotation extracts it to `next-session-task.md`. Handoff tasks must reference a backlog ID in the `backlog:` field — if none exists, create the item (default P1) before writing the handoff; `backlog: none` is valid only when the handover carries no continuing task. **HARD RULE: `file:` must NEVER point to `session-context.md`** — rotation blanks it, so the next session would find an empty file. Always write handoff data to a dedicated file in `docs/` (e.g., `docs/pending-<task>.md`) and point `file:` there. Never use `tmp/` — a handoff file is a tracked, pushed repository file (that is how it reaches the next machine), so it never carries a credential. This includes task lists, pending updates, execution plans — anything the next session needs to act on. **Restart-required verifications ARE handoff tasks** — if a fix can only be verified after restart (MCP changes, auth changes, hook changes), write it to next-session-task.md with explicit verification steps. Never put restart verifications in freeform "Recovery/Next session" text alone.
...
10. **Pending file carry-over (Layer 0b):** Check `PENDING_FILES:` in systemMessage. If `none`, skip. If listed, those `docs/pending-*.md` files exist and must be processed:
    ...
    - **Pending file header format:** `<!-- Action: present|act|triage|await-user-decision|defer|reference -->` as the first line, so the header stays invisible in rendered markdown. The bare `Action: x` form is still parsed for backward compatibility, but write the comment form. (CFG-482: the parser matched only the bare form, so every real pending file fell through to `triage` and `ACT_PENDING:` never fired.)
