...
- **Propagation check after edits — IMMEDIATELY, not at session end.** After editing any file that exists in both cfg-agent-fleet and agent-fleet (global/, hooks, setup/scripts/), check and propagate to the other repo in the same tool-call batch. Deployed files are symlinked to cfg — template-only fixes are dead on arrival until propagated. Don't wait for shutdown; don't say "done" before propagating. Run `bash sync.sh check` for drift detection.
...
| `lrn` | **Self-audit.** `lrn` alone = full audit. `lrn` + words = apply audit principles to what follows. Load `~/.claude/knowledge/learn-protocol.md`, then execute. Note: `lrn` = reliable trigger. `learn` (full word) only triggers if clearly directed at Claude or standalone. |
