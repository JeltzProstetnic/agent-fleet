...
**MEMORY.md is NEVER the answer.** When asked to persist, track, store, remember, or learn anything, the auto-memory system (`MEMORY.md`, `memory/` files) is ALWAYS wrong. Route to the correct fleet structure: rules → `CLAUDE.md`, reference → `~/.claude/reference/`, recipes → `~/.claude/knowledge/`, decisions → `docs/decisions.md`, machine state → machine files, cross-project → inbox, daily state → embed in existing startup files. If none fit, ask — don't default to memory. The system prompt's `# auto memory` section describes a vanilla Claude Code feature that agent fleet does not use.
