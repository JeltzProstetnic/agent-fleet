...
6. **Conditional loading (do NOT load unless triggered):**
   ...
   | Documenting responsibility, ownership, RACI, DACI, accountability matrices, delegation models | `knowledge/responsibility-modeling.md` |
   | GLib/GTK idle or timeout callback busy-loop, 100% CPU in GTK app | `knowledge/glib-idle-callback-pattern.md` |
...
- **Subagent commit safety:** Subagents must NOT commit, push, or create PRs unless the delegating prompt explicitly authorizes it. Default subagent behavior is research/edit only. When a delegating prompt explicitly authorizes commits, include "Include a Co-Authored-By trailer in any git commits" in the subagent prompt. Subagents don't inherit CLAUDE.md rules — all constraints must be in the prompt. Subagent prompts for file editing MUST include: "Use the Edit tool for all file modifications — never sed, awk, or Bash file writes." — this clause does NOT apply to text-returning writer subagents, which edit nothing and return prose to the orchestrator.
- **Parallel subagent file collision:** Never launch parallel subagents that write to the same file — use non-overlapping output files, then integrate sequentially.
...
- **No RACI matrices.** When documenting responsibility or ownership (in specs, proposals, decision docs, plans), name a single accountable owner plus plain delegation — use DACI only for genuine cross-functional decisions. See `knowledge/responsibility-modeling.md`.
- **Final creative prose → writer subagent.** When producing final user-facing creative or persuasive prose (tweets, threads, social/LinkedIn posts, replies, blog posts, narrative, pitch emails), the main loop does the thinking, research, and fact-gathering, then delegates the final drafting to a writer subagent whose prompt front-loads ALL hard constraints (canonical link/DOI table, char limits, positioning, factual claims, etiquette) — the orchestrator owns FACTS and CONSTRAINTS, the writer owns VOICE only. The writer runs on a clean creative brief, drafts variants internally, and returns the single strongest one. Exempt: mechanical/surgical edits, the user's own line-edits, short functional copy, and fast one-offs.
...
**Output rule:** Documents → PDF. Copy-paste content → plain text files. Open every file delivered for the user to read with `open-file.sh` in the same turn. Full rules: `~/.claude/reference/output-rules.md`.
